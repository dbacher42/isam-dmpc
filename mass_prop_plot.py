...
for k in range(h):
    ...
    lam_k = lambda_schedule(k, dt, lam_min=1.0, lam_max=500.0, gamma=0.9)
    # running cost
...
mass = np.zeros(h)
rho_x_hat = np.zeros(h)
rho_y_hat = np.zeros(h)
Jzz_hat = np.zeros(h)   
...
for k in range(h):
    ...
    G_block = np.zeros((3, 4))

    G_block[0, 0] = F[0]
    G_block[1, 0] = F[1]
    G_block[2, 0] = F[2]

    G_block[0, 1] = x_hat[6]**2
    G_block[1, 1] = acc[6]
    G_block[2, 1] = -acc[5]

    G_block[0, 2] = acc[6]
    G_block[1, 2] = x_hat[6]**2
    G_block[2, 2] = acc[4]
    ...
    mass[k]    = 1 / tht_hat[0]
    rho_x_hat[k] = tht_hat[1]
    rho_y_hat[k] = tht_hat[2]
    Jzz_hat[k]   = tht_hat[3] / tht_hat[0]
